 ...
 *
 * IN THE SHARED PACKAGE because the same document is built in two places:
 * the browser, when a customer or an employee presses "Download PDF", and
 * the API, when a plan is emailed to a visitor who opened or chose it. It
 * touches neither the DOM nor the database — `build()` hands back bytes, and
 * whoever called it decides whether they become a download or an attachment.
 */
...
export class PdfDocument {
  ...
  /** Serialise every page into the bytes of the file. */
  build(): Uint8Array {
    /**
