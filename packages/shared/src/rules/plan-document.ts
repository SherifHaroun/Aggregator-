 ...
 * screen.
 *
 * Built here, in the shared package, because the SAME file is downloaded from
 * the browser and emailed by the API: one renderer, so the PDF in a visitor's
 * inbox is byte-for-byte the one an employee would download for them.
 */

import { MAX_INSURABLE_AGE, NOT_SOLD_AT_AGE_LABEL } from '../config/business-rules.js';
import {
  PROVIDER_LIST_DOWNLOAD_LABEL,
  PROVIDER_LIST_PANEL_TITLE,
  providerListPanelSubtitle,
} from '../config/medical-benefits.js';
import type { ComparisonPlanResult } from '../types/comparison-results.js';
import { formatNumber } from './comparison-engine.js';
import { PdfDocument, rgb, widthOf, wrap, type Rgb } from './pdf-writer.js';
import {
...
export interface PlanDocumentInput {
  ...
  /**
   * The ABSOLUTE address a network's provider list is opened from, keyed on
   * the network. Written into the PDF, which leaves the site — so the caller,
   * who knows which origin it lives on, hands it in rather than the renderer
   * guessing. Without it a network is named in a plain row.
   */
  providerListUrl?: ((networkId: string) => string) | null;
}
...
/** The finished file, and the name it should be saved or attached under. */
export interface PlanDocumentFile {
  bytes: Uint8Array;
  filename: string;
}

/** Build the document. Returns the bytes and the name it should be saved under. */
export function renderPlanDocument(input: PlanDocumentInput): PlanDocumentFile {
  const { plan } = input;
  ...
  if (plan.medicalNetworkName) {
    ...
    if (plan.medicalNetworkId && plan.medicalNetworkHasProviderList && input.providerListUrl) {
      providerListPanel(doc, plan.medicalNetworkName, input.providerListUrl(plan.medicalNetworkId));
    } else {
